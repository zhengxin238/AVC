...
from from_db import read_from_db_sone
...
def integrataed_single_entry_dfList(cleint_name, db_name, collection_name):
    df = read_from_db_sone.get_stepone_df(cleint_name, db_name, collection_name)

    list_of_chunked_fds = read_from_db_sone.chunk_dataframe(df)
    dfs = read_from_db_sone.get_resultdf_original(list_of_chunked_fds)

    normorlised_dfs = read_from_db_sone.get_normalised_result(dfs)
...
cleint_name = "mongodb://localhost:27017/"
db_name_from = "Altruistic_result"
db_name_to ="DataSet_9compares"
